...
\abstract{
What are exploits in fact? It is possible to define them as programs that run on an unanticipated computational structure, performing unexpected computation. Buffer overflows, uncontrolled format strings or SQL injections are just some classical examples of vulnerabilities that allow to achieve this goal. It is possible to see exploits for these flaws as input data used as bytecode on some kind of virtual machine. We will call this class of peculiar VMs \emph{weird machines}.
...
\begin{itemize}
\item By placing the TSS on a page boundary the upper part of the table is saved in the "next instruction" (i.e. the next TSS) to keep the ESP state, while the CR3 register is not overwritten. %See Figure \ref{TSS_fig}.
...
\bibitem{hund} R. Hund, T. Holz, F. C. Freiling: {\sl Return-oriented rootkits: bypassing kernel code integrity protection mechanisms}; In Proceedings of the 18th USENIX Security Symposium (2009), USENIX Association, pp. 383–398.

\bibitem{noncontroldata} S. Chen, J. Xu, E. C. Sezer, P. Gauriar, R. K. Iyer: {\sl Non-control-data attacks are realistic threats}; In Proceedings of the 14th on USENIX Security Symposium (2005), USENIX Association
